...
<div>
    ...
    @endif
        ...
        <div class="form-group">
            ...
            <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" type="text" name="phone"  wire:model="phone" required>
            @error('phone') <span class="text-red-500">{{ $message }}</span>@enderror
        </div>
        <div class="form-group">
            <label for="address">email* </label>
            <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline" type="email" name="email"   wire:model="email" required>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody>
                    ...
                    @foreach ($customers as $customer)
                        ...
                        <td class="border px-4 py-1">{{ $customer->phone}} </td>
                        <td class="border px-4 py-1"> {{ $customer->email }} </td>
                        <td class="border px-4 py-1">{{ $customer->address }}</td>
                        <td class="border px-2 py-1">{{ $customer->created_at }}</td>
                        <td class="border px-2 py-1">{{ $customer->updated_at }}</td>
                        <td class="border px-4 py-1">
                    {{--     @can('customers-delete')    --}}                     
                
                    <a href="{{ route('customers.edit', $customer->id) }}" class="bg-blue-500 hover:bg-blue-700 text-white font-bold px-4 py-3 rounded">Edit</a>
               ...
               {{--   
                    <button wire:click="edit({{ $customer->id }})" class="bg-blue-500 hover:bg-blue-700 text-white font-bold px-4 py-3 rounded">Modifier</button>
                         <button wire:click="delete({{ $customer->id }})" class="bg-red-500 hover:bg-red-700 text-white font-bold p-1 rounded">Supprimer</button>
